  ...
  <div class="dashboard-container" v-resize="containerResize">
      ...
      <ul v-for="item in menus" :key="item.label">
        <li class="menu-option" @click="menusOperations(item.operation)">
          <label class="menuitem"
            ><svg-icon :icon-class="item.iconClass" /><span
              class="menuitem text"
              >{{ item.label }}</span
            >
          </label>
    ...
    >
      <template v-for="(item, index) in tableHead">
        ...
        <el-table-column
          v-if="index === 3 && showUpdateDateItem"
          :key="index"
          width="50"
          :index="index"
          align="center"
    ...
    >
      <div class="checkbox-group-header">
        <van-checkbox
          class="grid-all-checkbox"
          @click="clickGridAllCheckBox()"
          v-model="allChecked"
          >{{
        ...
        >
          ...
          >
            ...
            >
              <van-checkbox
                v-show="
                  gridHoverItemIndex === index || indexList.includes(index)
                "
    ...
    ></video-preview>
    <el-divider class="grid-divider" content-position="center"
      ><i class="el-icon-folder-opened"></i>&nbsp;{{ summaries }}</el-divider
    >
    <el-pagination
      background
  ...
  methods: {
    ...
    downloadFile(copy) {
      ...
      if (totalSize > 0) {
        ...
        if (fileIds.length > 1 || this.rowContextData.isFolder) {
          if (copy) {
            this.copyDownloadLink(
              fileConfig.publicPackageDownloadUrl(this.shareId, fileIds)
            );
          } else {
        ...
        if (copy) {
          this.copyDownloadLink(
            fileConfig.publicDownloadUrl(this.shareId, this.rowContextData)
          );
        } else {
