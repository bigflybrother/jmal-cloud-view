<template>
  <div class="container" v-resize="containerResize">
      ...
      <div class="left-menu">
        ...
        <div class="classification">
          ...
          <van-cell
            title="视频"
            icon="video-o"
            @click="clickFileType('video', { isFolder: false })"
          />
          <van-cell
            title="音乐"
            icon="music-o"
            @click="clickFileType('audio', { isFolder: false })"
          />
          <van-cell title="文件传输" icon="upgrade" @click="uploadList" />
        </div>

        <div class="logout">
          <van-button round type="danger" @click.native="logout"
            >退出登录</van-button
          >
        </div>
      </div>
    ...
    <van-list
      ...
      @load="
        searchStatus && searchValue.length > 0
          ? searchFile(searchValue, true)
          : getFileList(true)
      "
    >
      <van-checkbox-group
        ...
        <van-cell-group>
          <van-sticky :offset-top="-1">
            ...
            <van-nav-bar
              ...
              :right-text="
                this.selectRowData.length !== this.fileList.length
                  ? '选择'
                  : '取消'
              "
              @click-left="leftMenu"
          ...
          <!--搜索结果-->
          <van-cell
            v-if="searchStatus && searchValue.length > 0 && fileList.length > 0"
          ...
          <van-cell
            v-if="
              searchStatus && searchValue.length > 0 && fileList.length === 0
            "
          >
          ...
          <van-cell
            ...
            class="list-item"
            :style="{
              background: selectIndexList.includes(index) ? '#90c2fcb5' : ''
            }"
            center
          ...
          >
            ...
            >
              <div
                ...
                :style="{
                  background: selectIndexList.includes(index)
                    ? '#90c2fcb5'
                    : '',
                  'border-radius': '15px'
                }"
              ...
              >
                ...
                <span :title="item.name" class="grid-item-text">{{
                  item.name
                }}</span>
              </div>
            </van-grid-item>
    ...
    >
      <ul
        ...
        :key="item.label"
        :class="{
          'menu-list-first': index === 0,
          'menu-list-last': index === menus.length - 1,
          'menu-list': index < menus.length - 1 && index > 0
        }"
      >
        <div v-if="index !== 0" style="border-bottom:1px solid #cccccc85"></div>
        <li
          :class="{ remove: item.operation === 'remove' }"
    ...
    >
      <van-cell
        v-if="rowContextData.name != null && !grid"
        :style="overlayContentClass"
        class="list-item overlay-content-class"
        center
        :is-link="rowContextData.isFolder"
      >
        <div>
          <van-row class="row-file">
            <van-col span="4" align="center" class="list-cell-icon">
            ...
            <van-col span="16" class="list-item-content">
              ...
              >
                <van-col span="24">
                  {{ formatTime(rowContextData.agoTime) }}&nbsp;&nbsp;&nbsp;{{
      ...
      >
        ...
        <span :title="rowContextData.name" class="grid-item-text">{{
          rowContextData.name
        }}</span>
      </div>
    </van-overlay>
...
export default {
  ...
  data() {
    return {
      ...
      audioCoverUrl:
        process.env.VUE_APP_BASE_API +
        "/view/cover?jmal-token=" +
        this.$store.state.user.token +
        "&id=",
      tabActive: 0,
      path: this.$route.query.path,
      fileList: [],
      pathList: [{ folder: "", index: 0 }, { folder: "+", index: 1 }],
      pagination: {
        pageIndex: 0,
        pageSize: 50,
        total: 0
      ...
      actionsMenus: [{ name: "新建文件夹" }],
      actionSortSheetShow: false,
      actionsSortMenus: [{ name: "名称" }, { name: "日期" }, { name: "大小" }],
      actionsSortName: "名称",
      showNewFolder: false,
      showRename: false,
      newFolderName: "新建文件夹",
  ...
  methods: {
    ...
    longPressDisplayLayer(item, offsetParent, touchClientX) {
      const background = this.selectIndexList.includes(
        this.rowContextData.index
      )
        ? "#90c2fcb5"
        : "";
    ...
    goBack() {
      if (this.pathList.length <= 1) {
        this.$router.push(
          `/_m?vmode=${this.vmode}&path=${encodeURIComponent(this.path)}`
        );
        return;
      }
      this.clickBack();
    ...
    loadData(res, onLoad) {
      if (onLoad) {
        res.data.forEach((file, index) => {
          file["index"] =
            (this.pagination.pageIndex - 1) * this.pagination.pageSize + index;
    ...
    statistics() {
      ...
      return (
        this.getShowSumFileAndFolder(this.fileList) +
        " " +
        this.getShowSumSize(totalSize)
      );
    },
    // 统计文件和文件夹
...
.tab-bottom {
  ...
  .van-tabbar-item {
    margin-top: 25px;
    animation: ordinary 0.4s forwards;
    -moz-animation: ordinary 0.4s forwards; /* Firefox */
    -webkit-animation: ordinary 0.4s forwards; /* Safari and Chrome */
    -o-animation: ordinary 0.4s forwards; /* Opera */
  }
}
...
.tab-bottom-iphoneX {
  ...
  .van-tabbar-item {
    animation: iphoneX 0.4s forwards;
    -moz-animation: iphoneX 0.4s forwards; /* Firefox */
    -webkit-animation: iphoneX 0.4s forwards; /* Safari and Chrome */
    -o-animation: iphoneX 0.5s forwards; /* Opera */
  }
}
...
.tab-bottom-iphoneXS {
  ...
  .van-tabbar-item {
    ...
    -moz-animation: iphoneX 0.4s forwards; /* Firefox */
    -webkit-animation: iphoneX 0.4s forwards; /* Safari and Chrome */
    -o-animation: iphoneX 0.4s forwards; /* Opera */
  }
}
