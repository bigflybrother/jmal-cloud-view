<template>
  <div>
    ...
    <div class="dir-tools" style="min-width: 272px;">
      <el-button-group>
        <el-button
          :class="lightTheme ? 'light-button' : 'dark-button'"
          size="small"
    ...
    <div
      id="dir-tree"
      :style="{ width: contentsWidth + 'px', height: contentsHieght + 'px' }"
    ></div>
  </div>
</template>
<script>
...
export default {
  ...
  data() {
    return {
      ...
      fileMenus1: [
        {
          iconClass: "menu-addtab",
          label: "在新的标签打开",
          operation: "openFileOfNewTab"
        },
        // { iconClass: 'menu-rename', label: '重命名', operation: 'rename' },
        { iconClass: "menu-remove", label: "删除", operation: "remove" }
      ],
  ...
  methods: {
        ...
        edit: {
          ...
          beforeEdit: function(event, data) {
            const isModifying = that.editableTabs.some(item => {
              return (
                data.node.data.path === item.name && "Modifying" === item.status
              );
            });
            if (isModifying) {
          ...
          edit: function(event, data) {
            let input = data.node.span.querySelector(
              ".fancytree-title .fancytree-edit-input"
            );
            let width = input.style.width.substring(
              0,
              input.style.width.length - 2
            );
            input.style.width = parseInt(width) + 50 + "px";
            if (
              input.value &&
              input.value.length - 1 > data.node.data.suffix.length
            ) {
    ...
    contextmenu(vnode) {
      ...
      if (this.contextData.isFolder) {
        ...
        if (this.contextData.isFolder === undefined) {
          ...
          const tabExist = this.editableTabs.some(
            item => item.name === this.contextData.path
          );
    ...
    editCreateNode(node, isFolder) {
      if (node === null || node === undefined) {
        this.tree
          .getRootNode()
          .editCreateNode("child", { title: "", folder: isFolder });
        return;
      }
      if (node.folder) {
    ...
    upperLeve() {
      ...
      let path = this.currentPath.substring(
        0,
        this.currentPath.length - title.length
      );
      api
        .upperLevelList({
          username: this.$store.state.user.name,
    ...
    createFile(fileName, row, node) {
      ...
      if (node.parent.folder) {
        ...
        if (parent.path) {
          parentPath += parent.path.substring(
            0,
            parent.path.length - parent.name.length
          );
        } else {
      ...
      api
        ...
        .then(res => {
          ...
          let path =
            "/" +
            activeNodeData.path.substring(
              0,
              activeNodeData.path.length - activeNodeData.name.length
            );
          this.loadTreeData(
            { name: activeNodeData.name, path: path },
            true,
            activeNodeData
          );
        })
    ...
    nameValid(newFileName, row, node) {
      if (newFileName) {
        ...
        const findIndex = node.parent.children.findIndex(
          item => newFileName === item.data.name
        );
        if (findIndex > -1) {
          let msg = "该文件已存在";
          if (row.isFolder) {
    ...
    nodeRename(newFileName, row, node) {
      if (newFileName) {
        ...
        let strFileName = newFileName.substring(
          0,
          newFileName.length - newExt.length
        );
        if (!row.isFolder) {
      ...
      api
        ...
        .then(res => {
          if (res.data) {
            ...
            row.path =
              row.path.substring(0, row.path.length - row.oldName.length) +
              newFileName;
            node.setTitle(newFileName);
            this.addRef(node);
    ...
    convert(data) {
      if (data && data.length > 0) {
        this.currentPath = data[0].path.substring(
          0,
          data[0].path.length - data[0].name.length
        );
      }
      if (this.currentPath.length === 0) {
        this.currentPath = "/";
