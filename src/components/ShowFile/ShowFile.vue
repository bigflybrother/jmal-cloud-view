  ...
  <div>
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="small" @click="fileTreeAndNewFolder"
    ...
    >
      <el-input
        ...
        class="dialog-msg"
        @focus="renameInputFocus($event, '')"
        :clearable="true"
        @blur="setInputBlur()"
        @keyup.enter.native="createTextFile(newTextFileName)"
      ></el-input>
      <span slot="footer" class="dialog-footer">
        <el-button size="small" @click="newTextFileDialog = false"
          >取消</el-button
        >
        <el-button
          size="small"
          type="primary"
          @click="createTextFile(newTextFileName)"
          v-loading="createTextFileLoading"
          >确定</el-button
        >
      </span>
    </el-dialog>
  ...
  methods: {
          ...
          if (dragIndex !== throughRow.rowIndex) {
            ...
            if (
              ...
              numberFilesCopy.querySelector(".operate").style.display =
                "inline";
              let targetFolder =
                numberFilesCopy.querySelector(".target .folder");
    ...
    goBack() {
      if (this.pathList.length <= 1) {
        this.$router.push(
          `/?vmode=${this.vmode}&path=${encodeURIComponent(this.path)}`
        );
        return;
      }
      this.lastLink();
    ...
    contextmenuClick(operation) {
      switch (operation) {
        ...
        case "refresh":
          ...
          break;
        case "createTextFile":
          this.newTextFileName = this.getNewFileName(
            this.fileList,
            "未命名文件"
          );
          ...
          if (operating === "move") {
            ...
            if (this.$refs.fileListTable.tableSelectData.length === 1) {
              this.fileList.splice(
                this.$refs.fileListTable.tableSelectData[0].index,
                1
              );
            } else {
              this.getFileList();
